...
version-selection logic below): `t = [0.02917, 0.060, 0.03994]` (raw solve, **no manual offset**). Earlier backups were removed on
...
3. **t.y (vertical) — weakly observable from boards** (plane normals stay nearly coplanar for upright
   ...
   depth-based solve (`solve_ty_residual.py`) returned Δt.y = −0.3 mm at dy −30 mm (noise level). Manual offsets (dy −30/−20 mm)
   were tried and **rejected by final eye review — the raw solve stands, no offset**;
   vertical is closed.
