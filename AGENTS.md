...
version-selection logic below): `t = [0.02917, 0.040, 0.03994]`. Earlier backups were removed on
...
3. **t.y (vertical) — weakly observable from boards** (plane normals stay nearly coplanar for upright
   ...
   depth-based solve (`solve_ty_residual.py`) returned Δt.y = −0.3 mm at dy −30 mm → **both
   independent judges (eye + depth cloud) agree on dy −30 mm** (final eye pass
   settled on **dy −20 mm**); vertical is closed.
